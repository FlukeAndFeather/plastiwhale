### SCRATCHPAD FOR TRYING TO INTEGRATE CHOY
...
#I could make lambda by averaging all the predicition values, but that seems like it would defeat the purpose
#of having made such nice distributions 
...
simulate_h2o_plastic <- function(feeding_simulation, retention) {  
  feeding_simulation %>%
    left_join(predictions, by = "depth_bucket") %>% 
    mutate(all_plastic = daily_lunges * engulf_m3_skr * plastic_conc) %>% 
...
} #something about this doesnt make sense. doesnt it calculate for both krill and fish? 
# if it does, then i shouldnt be /0.00008 
...
total_plastic <- simulate_h2o_plastic(feeding_simulation = lunges_and_length, retention =  0.75) %>%  #can be .25, .5, .75
  left_join(simulate_prey_plastic(feeding_simulation = lunges_and_length, "krill", prey_plastic_conc = 0.05), by = "simulation_id") #change prey and prey_plastic_conc
